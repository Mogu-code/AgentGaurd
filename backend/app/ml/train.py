...
def main():
    ...
    behavioral_cols = [X.columns.get_loc(c) for c in ["tool_calls_in_session", "session_duration_s", "retry_count", "transaction_velocity_s"] if c in X.columns]
    ...
    merchant_cols = [X.columns.get_loc(c) for c in ["merchant_known", "merchant_switching"] if c in X.columns]
