    ...
    <div className="app-container">
      ...
      <div className="sidebar">
        ...
        <div className="nav-menu">
          ...
          >
            <ShieldCheck size={20} /> Security View
          </div>
          <div 
      ...
      <div className="main-content">
          ...
          <div>
            ...
            <div className="simulation-actions">
              <button className="card button sim-btn" onClick={() => runSimulation('legitimate')} disabled={loading}>
                ...
                <span>Authorized ₹45,000 laptop</span>
              </button>
              ...
              <button className="card button sim-btn" onClick={() => runSimulation('merchant_substitution')} disabled={loading}>
                <div className="flex items-center gap-2"><AlertTriangle size={18} className="text-red"/> Merchant Substitution</div>
                <span>Attempts purchase at ShadyTechStore</span>
              </button>
        ...
        {activeTab === 'security' && (
          <div>
            <div className="header">
              <h1>Architecture & Security</h1>
              <p>The LLM never has direct payment authority.</p>
            </div>
            <div className="card">
              <pre className="code-block text-sm" style={{lineHeight: '1.5'}}>
{`1. NL Input         User: "Buy a laptop under ₹70k"
   |
2. LLM Extraction   Ollama qwen3:4b parses intent
   |                [VALIDATION] Schema checked. Fallback if hallucinated.
   v
3. Capability       { max_amount: 70000, qty: 1 } stored immutably.
   ========================================================================
                      LLM INFLUENCE BOUNDARY
   ========================================================================
   |
4. Agent Action     Agent requests a payment (e.g. ₹65000, Qty 3)
   |
5. Policy Engine    Deterministic checks against Capability.
   |                [REPLAY] Checked. [VELOCITY] Checked.
   v
6. Risk Engine      ML evaluates behavior & history against session context.
   |
7. Decision         ALLOW / STEP_UP / BLOCK
   |
8. Gateway          Razorpay invoked ONLY on ALLOW.`}
              </pre>
            </div>
          </div>
        )}
      ...
      <div className="flow-step">
        ...
        <div className="flow-content">
          <div className="flow-card">
            ...
            <div className="code-block">"{ext.original_intent || 'Buy a laptop under ₹70,000'}"</div>
          </div>
        </div>
      </div>
        ...
        <div className="flow-content">
          <div className="flow-card highlight">
            <div className="flow-header flex justify-between">
              <span>LLM Extraction ({ext.provider || 'ollama'} / {ext.model || 'qwen3:4b'})</span>
              <span className="badge badge-green">{ext.validation_status === 'success' ? 'VALIDATED' : 'FALLBACK'}</span>
            </div>
            <div className="data-row"><span className="data-label">Method</span><span className="data-value">{ext.method === 'llm' ? 'Generative AI' : 'Rule-Based Fallback'}</span></div>
            <div className="data-row"><span className="data-label">Max Amount</span><span className="data-value">₹{ext.structured_intent?.max_amount?.toLocaleString() || '70,000'}</span></div>
            <div className="data-row"><span className="data-label">Max Quantity</span><span className="data-value">{ext.structured_intent?.max_quantity || '1'}</span></div>
          </div>
        </div>
      </div>
        ...
        <div className="flow-content">
          <div className="flow-card">
            <div className="flow-header flex justify-between">
              ...
              <span className="badge badge-gray">{rzpMode.toUpperCase()} MODE</span>
            </div>
            {isBlock ? (
              <div className="text-secondary" style={{marginTop: '0.5rem'}}>Payment blocked. No Razorpay API call made.</div>
            ) : (
              <div>
